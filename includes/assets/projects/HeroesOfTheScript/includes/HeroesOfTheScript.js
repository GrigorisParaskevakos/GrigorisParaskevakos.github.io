...
function HeroesOfTheScript() {
  $.alert({
    title: "The game says:",
    content: "Welcome to Heroes Of The Script!"
  });
  ...
  alert(
    "Watch out as you move forward for the Enemies...\n\n" +
      enemy[0] +
      "s\n" +
      "\n" +
      enemy[1] +
  ...
  while (flag2 == true) {
    ...
    if (move == "W" || move == "w") {
      ...
      if (chanceToFight > 60) {
        // if number > 60% nothing happens
        alert("You are walking...in peace!");
      } else if (chanceToFight <= 60) {
        // else is number < or equal to 60% players finds an enemy.
        ...
        while (flag5 == true) {
          if (move == "A" || move == "a") {
            //when players finds an enemy if pressed A we have a fight.
            flag5 = false;
            ...
            while (flag4 == true) {
              ...
              if (pL > 0) {
                ...
                if (EnL <= 0) {
                  ...
                  if (xp >= 10) {
                    ...
                    if (lvl == 2) {
                      ...
                      //when user completes level 5 the game is over
                      alert(
                        "Congratulations! " +
                          nameHero +
                          "\n" +
                      ...
                      if (x == false) {
                        // if user cancels then goodbye’s user
                        flag4 = false;
                        flag2 = false;
                        alert(
                          "Thank you for playing Heroes of The Script! Bye."
                        );
                        window.close();
                      } else if (x == true) {
                        //else plays again from start with initial level , experience and life values.
                ...
                // checks if player lost’s all life points
            ...
            //when players finds an enemy if pressed R there is a 50% chance to have a fight or run away and don’t fight.
            flag5 = false;
            var chanceToRun = Math.floor(Math.random() * 2); // random number 50% chance (0 or 1)
            if (chanceToRun == 0) {
              // if random number is 0 we have a fight
            ...
            } else if (chanceToRun == 1) {
              //if random number is 1 we don't have a fight and player also gets 10 life points
              flag5 = false;
              pL = pL + 10; //player gets 10 life points
              alert(
                "Ouf you run away safe this time...\n" + "+10 LifePoints added!"
              );
            }
          } else if (
            move !== "A" ||
            ...
            //error handling forces user to choose only between A or R keys when enemy appears.
            move = prompt(
              "Please write A to attack \n or R to try to run away.."
            );
          }
        }
        temp1dD = temp1dD + dD;
